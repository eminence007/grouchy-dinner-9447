...
const Graph = () => {
  ...
  useEffect(() => {
    ...
    const carbsCal = macronutrients.Carbohydrates
      ? macronutrients.Carbohydrates.reduce(
          (acc, carb) => acc + carb.weight * 4,
          0
        )
      : 0;
    const proteinCal = macronutrients.Protein
      ? macronutrients.Protein.reduce(
          (acc, protein) => acc + protein.weight * 4,
          0
        )
      : 0;
    const fatCal = macronutrients.Fat
      ? macronutrients.Fat.reduce((acc, fat) => acc + fat.weight * 9, 0)
      : 0;
    const highlightCal = foodData.HighlightedNutrients
      ? foodData.HighlightedNutrients.reduce(
          (acc, nutrient) => acc + nutrient.weight * 4,
          0
        )
      : 0;
    ...
    datasets: [
      {
        label: "Food Calories",
        data: [
          calculateTotalEnergy(foodData),
          calculateTotalEnergy(foodData, 1),
          calculateTotalEnergy(foodData, 2),
        ],
      ...
      },
      {
        ...
        data: [
          calculateTotalEnergy(exerciseData),
          calculateTotalEnergy(exerciseData, 1),
          calculateTotalEnergy(exerciseData, 2),
        ],
